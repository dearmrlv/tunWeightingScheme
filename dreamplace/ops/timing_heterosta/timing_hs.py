...
class TimingOpt(nn.Module):
    def __init__(self, timer, net_names, pin_names, flat_netpin,
                 ...
                 pin2net_map,  pin2pin_net_weight,
    ...
    def update_net_weights(self, max_net_weight=np.inf, n=1):
        ...
        """
        if self.net_weighting_scheme == "lilith": scm = 1
        ...
        try:
            ...
            pin2node_map_t = torch.from_numpy(self.pin2node_map)
            pin2net_map_t = torch.from_numpy(self.pin2net_map)
    ...
    def report_wns_hold(self):
        """
        @brief report WNS in the design -- hold mode
        """
        return timing_hs_cpp.report_wns_hold(
            self.timer.raw_timer, bool(self.use_cuda))
    
    def report_tns_hold(self):
        """
        @brief report TNS in the design -- hold mode
        """
        return timing_hs_cpp.report_tns_hold(
            self.timer.raw_timer, bool(self.use_cuda))
